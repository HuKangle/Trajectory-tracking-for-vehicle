function [sys,x0,str,ts] = MPC3(t,x,u,flag)
%***************************************************************%
    switch flag,
...
sizes.NumOutputs     = 43;  %S��������������������������������
sizes.NumInputs      = 38; %S����ģ����������ĸ�������CarSim�������????????????
sizes.DirFeedthrough = 1;  %ģ���Ƿ����ֱ�ӹ��?(direct feedthrough). 1 means there is direct feedthrough.
...
[y, e] = func_RLSFilter_Calpha_r('initial', 0.95, 10, 10);
    ...
    global CostWeights; 
    CostWeights.Wephi   = 200; %state vector =[beta,yawrate,e_phi,s,e_y]
    CostWeights.Wey     = 400;
    CostWeights.Weyd    = 2;
    CostWeights.Wepsid  = 2;
    CostWeights.Ddeltaf = 0.1;
    CostWeights.deltaf  = 5;
    ...
    Constraints.DPhimax = 10*pi/180;  %  ������ƫ��60deg
...
%     [StateSpaceModel] = func_RigidbodyDynamicalModel_FOH_Extended(VehiclePara, MPCParameters, Vel, CafHat, CarHat);
    % [StateSpaceModel] = func_RigidbodyModel_FOH_Matrix_ROLL_YALMIP(VehiclePara, MPCParameters, Vel, CafHat, CarHat);
    Station =  PrjP.yr;  
    Np          = MPCParameters.Np;
    Eymax       = zeros(Np,1);
    Eymin       = zeros(Np,1);     
...
%     L = VehiclePara.L;
% 
%     Caf = VehiclePara.CafHat;
%     Car = VehiclePara.CarHat;
% 
%     M   = VehiclePara.m;
%     Iz  = VehiclePara.Iz;
%     forwa = (Kappar) * (L - b * K(3) - M * Vel^2 / L * (b/ Caf + a * K(3)/ Car - a/ Car));
...
%% obtain the SWA from Stanley method
       ...
       solution     = 1;
    %  fwa_sol      = fwa_sol - CostWeights.K_g * (x0 - x_nom1);     
...
% sys = [Ctrl_SteerSW; t_Elapsed; PosX; PosY; PosPsi; Station; Roll; e_psi; e_y; YZPM; LTR_T; LTR; Vy;  Beta; yawrate]; %

%sys = [Ctrl_SteerSW; t_Elapsed; PosX; PosY; PosPsi; Station; PSI; e_psi; e_y; cost; LTR_T; LTR; Vy; Beta; yawrate; fwa_sol; fwa_e; err; ENV; x_nom1; x0(6); U(2,9:15)';U(2,4:8)'; Ay;U(2,1:3)'; solution;Dist; tire_parameter]; %
sys = [Ctrl_SteerSW; t_Elapsed; PosX; PosY; PosPsi; Station; PSI; e_psi; e_y; cost; LTR_T; LTR; Vy; Beta; yawrate; fwa_sol; fwa_e; err; U(2,1:5)'; solution;Dist; tire_parameter]; %
