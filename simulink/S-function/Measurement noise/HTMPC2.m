function [sys,x0,str,ts] = HTMPC2(t,x,u,flag)
%***************************************************************%
...
%     obj.gcc.p
%     obj.gcc.k
    mrpi = load("E:\carsim\CARSIM2020\Extensions\Simulink\chapter8_rollover_prevention_terrain\DoubleLineShift\mrpi_obj");
    obj = mrpi.obj;
    Np          = MPCParameters.Np;
    Eymax       = zeros(Np,1);
    Eymin       = zeros(Np,1);     
...
%***************************************************************%  
    ...
    VehStatemeasured.phi_dot = (round(10*ModelInput(6))/10)*pi/180+ ModelInput(41); %Unit��deg/s-->rad/s
